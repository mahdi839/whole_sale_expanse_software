    ...
    <div class="space-y-4">
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-5">
            <form method="GET" action="{{ route('sales.index') }}">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-3.5">
                    ...
                    <select name="status" class="h-9 px-3 text-sm bg-gray-50 border rounded-lg">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('sales.create') }}"
                       class="h-9 px-4 bg-blue-600 text-white rounded-lg text-sm flex items-center gap-1">
                        + New Sale
                    </a>
                </div>
            </form>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div class="bg-white border rounded-xl p-4">
                ...
                <p class="text-xl font-semibold">{{ number_format($totals->total_sales ?? 0) }}</p>
            </div>
            <div class="bg-white border rounded-xl p-4">
                <p class="text-xs text-gray-400 uppercase">Grand Total</p>
                <p class="text-xl font-semibold text-blue-600">৳{{ number_format($totals->total_amount ?? 0, 2) }}</p>
            </div>
            <div class="bg-white border rounded-xl p-4">
                <p class="text-xs text-gray-400 uppercase">Total Paid</p>
                <p class="text-xl font-semibold text-green-600">৳{{ number_format($totals->total_paid ?? 0, 2) }}</p>
            </div>
            <div class="bg-white border rounded-xl p-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-5 py-20 text-center text-gray-400">
                                    No sales found.
                                    <a href="{{ route('sales.create') }}" class="text-blue-600">Create first sale</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
