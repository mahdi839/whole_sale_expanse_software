<x-app-layout>
    ...
    <div class="space-y-4">
            ...
            <form method="GET" action="{{ route('missing-products.index') }}">
                ...
                <div class="flex flex-col sm:flex-row gap-2">
                    ...
                    @canany(['manage products', 'view products'])
                        <button type="submit" formaction="{{ route('missing-products.export.pdf') }}" class="h-10 px-4 text-sm font-medium text-red-700 bg-red-50 border border-red-200 rounded-lg inline-flex items-center justify-center">PDF</button>
                    @endcanany
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="bg-white border border-gray-200 rounded-xl px-5 py-4 flex items-center justify-between">
                <p class="text-sm text-gray-500">Filtered purchase value</p>
                <p class="text-lg font-semibold text-amber-600">{{ number_format($totalPurchaseValue ?? 0, 2) }}</p>
            </div>
        </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 border-b">
                            ...
                            <th class="px-5 py-3 text-right text-xs font-medium text-gray-400">Purchase Rate</th>
                            <th class="px-5 py-3 text-right text-xs font-medium text-gray-400">Purchase Value</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($missingProducts as $entry)
                            <tr>
                                ...
                                <td class="px-5 py-3 text-right">{{ number_format($entry->purchase_rate, 2) }}</td>
                                <td class="px-5 py-3 text-right font-medium text-amber-700">{{ number_format($entry->purchase_value, 2) }}</td>
                                <td class="px-5 py-3">
                                    @if($entry->document)
