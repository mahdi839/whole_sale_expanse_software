<x-app-layout>
    ...
    <div class="space-y-4">
        ...
        @if (session('error'))
            <div
                class="flex items-center gap-2.5 px-4 py-3 text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl">
                <svg class="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-3.5">
                    ...
                    <select name="return_status"
                        ...
                        @foreach (['pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected'] as $v => $l)
                            <option value="{{ $v }}"
                                {{ ($filters['return_status'] ?? '') === $v ? 'selected' : '' }}>{{ $l }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="return_type"
                        ...
                        @foreach (['refund' => 'Refund', 'exchange' => 'Exchange', 'credit' => 'Credit'] as $v => $l)
                            <option value="{{ $v }}"
                                {{ ($filters['return_type'] ?? '') === $v ? 'selected' : '' }}>{{ $l }}
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="grid grid-cols-3 gap-3">
            @foreach ([['label' => 'Total Returns', 'value' => number_format($totals->total_returns ?? 0), 'color' => 'text-gray-800'], ['label' => 'Total Qty Returned', 'value' => number_format($totals->total_qty ?? 0, 2), 'color' => 'text-gray-800'], ['label' => 'Total Return Amount', 'value' => '৳' . number_format($totals->total_return_amount ?? 0, 2), 'color' => 'text-red-600']] as $card)
                <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-5">
                    <p class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1.5">{{ $card['label'] }}
                    </p>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 bg-gray-50">
                            <th class="text-left px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide">
                                Reference</th>
                            <th
                                class="text-left px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide hidden md:table-cell">
                                Original Sale</th>
                            <th class="text-left px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide">
                                Customer</th>
                            <th
                                class="text-left px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide hidden lg:table-cell">
                                Product</th>
                            <th
                                class="text-right px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide hidden md:table-cell">
                                Amount</th>
                            <th class="text-left px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide">
                                Status</th>
                            <th class="text-right px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wide">
                                Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-50/60 transition-colors">
                                ...
                                <td class="px-5 py-3.5 align-top">
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">
                                        {{ optional($return->date)->format('d M Y') }}</p>
                                </td>
                                ...
                                <td class="px-5 py-3.5 align-top hidden lg:table-cell">
                                    @foreach ($return->items as $item)
                                        <p class="font-medium text-gray-800"><strong>Product:
                                ...
                                <td
                                    class="px-5 py-3.5 align-top text-right font-medium text-red-600 tabular-nums hidden md:table-cell">
                                ...
                                <td colspan="8" class="px-5 py-20 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-400">
                                        ...
                                        <p class="text-sm">No returns found.
                                            <a href="{{ route('sale-returns.create') }}"
                                                class="text-blue-600 hover:underline">Add your first return.</a>
                                        </p>
                                    </div>
                                </td>
                            </tr>
