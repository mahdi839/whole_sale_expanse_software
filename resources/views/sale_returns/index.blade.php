<x-app-layout>
    ...
    <div class="space-y-4">
            ...
            <form method="GET" action="{{ route('sale-returns.index') }}">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 mb-3.5">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="w-full pl-9 pr-3 h-10 text-sm bg-gray-50 border border-gray-200 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition"
                        />
                    </div>
                    ...
                    <input
                        ...
                        name="date"
                        value="{{ $filters['date'] ?? '' }}"
                        class="w-full px-3 h-10 text-sm bg-gray-50 border border-gray-200 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition"
                    />
                    ...
                    >
                        ...
                        @foreach (['pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected'] as $v => $l)
                            <option value="{{ $v }}" {{ ($filters['return_status'] ?? '') === $v ? 'selected' : '' }}>
                                {{ $l }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    >
                        ...
                        @foreach (['refund' => 'Refund', 'exchange' => 'Exchange', 'credit' => 'Credit'] as $v => $l)
                            <option value="{{ $v }}" {{ ($filters['return_type'] ?? '') === $v ? 'selected' : '' }}>
                                {{ $l }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2">
                    <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                        <button
                            type="submit"
                            class="h-10 px-4 text-sm font-medium text-white bg-gray-800 rounded-lg transition w-full sm:w-auto"
                        >
                            Filter
                        </button>
                        ...
                        <a
                            ...
                            class="h-10 px-4 inline-flex items-center justify-center text-sm font-medium text-white bg-cyan-600 rounded-lg hover:bg-cyan-700 transition w-full sm:w-auto"
                        >
                            Reset
                        </a>
                    </div>
                    ...
                    <div class="sm:ml-auto flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                        <a
                            href="{{ route('sale-returns.export', request()->query()) }}"
                            class="h-10 px-4 inline-flex items-center justify-center gap-2 text-sm font-medium text-green-700 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition w-full sm:w-auto"
                        >
                        ...
                        <a
                            ...
                            class="h-10 px-4 inline-flex items-center justify-center gap-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition w-full sm:w-auto"
                        >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            @foreach ([
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-5">
                    ...
                    <p class="text-xl font-semibold {{ $card['color'] }} break-words">{{ $card['value'] }}</p>
                </div>
            @endforeach
        </div>
        ...
        <div class="sm:hidden space-y-3">
            @forelse($returns as $return)
                <div class="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <a
                                href="{{ route('sale-returns.show', $return) }}"
                                class="inline-block px-2 py-0.5 rounded-md bg-orange-50 text-orange-700 text-xs font-mono font-medium hover:bg-orange-100 transition break-all"
                            >
                                {{ $return->reference }}
                            </a>
                    ...
                    <div>
                        <p class="text-xs text-gray-400 mb-1">Original Sale</p>
                        @if ($return->sale)
                            <a
                                href="{{ route('sales.show', $return->sale) }}"
                                class="inline-block px-2 py-0.5 rounded-md bg-violet-50 text-violet-700 text-xs font-mono font-medium hover:bg-violet-100 transition break-all"
                            >
                    ...
                    <div class="flex flex-col gap-2 pt-1">
                        @if ($return->return_status === 'pending')
                            <form
                                method="POST"
                                action="{{ route('sale-returns.approve', $return) }}"
                                onsubmit="return confirm('Approve this return? Stock and financials will be updated.')"
                            >
                                @csrf
                                <button
                                    type="submit"
                                    class="w-full inline-flex items-center justify-center gap-1 h-9 px-3 text-xs font-medium text-green-700 bg-green-50 border border-green-100 rounded-lg hover:bg-green-100 transition"
                                >
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <a
                                href="{{ route('sale-returns.edit', $return) }}"
                                class="inline-flex items-center justify-center gap-1.5 h-9 px-3 text-xs font-medium text-blue-700 bg-blue-50 border border-blue-100 rounded-lg hover:bg-blue-100 transition"
                            >
                            ...
                            <form
                                method="POST"
                                action="{{ route('sale-returns.destroy', $return) }}"
                            ...
                            >
                                ...
                                <button
                                    type="submit"
                                    class="w-full inline-flex items-center justify-center gap-1.5 h-9 px-3 text-xs font-medium text-red-700 bg-red-50 border border-red-100 rounded-lg hover:bg-red-100 transition"
                                >
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            @empty
                <div class="bg-white border border-gray-200 rounded-xl px-5 py-16 text-center">
                                    ...
                                    <div class="flex items-center justify-end gap-1.5">
                                        @if ($return->return_status === 'pending')
                                            ...
                                            >
                                                ...
                                                <button
                                                    type="submit"
                                                    class="inline-flex items-center gap-1 h-7 px-2.5 text-xs font-medium text-green-700 bg-green-50 border border-green-100 rounded-lg hover:bg-green-100 transition"
                                                >
                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                        <path d="M5 13l4 4L19 7" />
                                                    </svg>
                                        ...
                                        <a
                                            href="{{ route('sale-returns.edit', $return) }}"
                                            class="inline-flex items-center gap-1.5 h-7 px-2.5 text-xs font-medium text-blue-700 bg-blue-50 border border-blue-100 rounded-lg hover:bg-blue-100 transition"
                                        >
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        ...
                                        >
                                            ...
                                            <button
                                                type="submit"
                                                class="inline-flex items-center gap-1.5 h-7 px-2.5 text-xs font-medium text-red-700 bg-red-50 border border-red-100 rounded-lg hover:bg-red-100 transition"
                                            >
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                    <path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                </svg>
