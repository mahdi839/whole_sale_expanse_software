    ...
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <form method="GET" action="{{ route('carry-man-work-logs.index') }}" class="flex items-center gap-2 w-full sm:max-w-md">
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search carry man, memo or marka..."
            ...
            @canany(['manage carry man work logs', 'view carry man work logs'])<a href="{{ route('carry-man-work-logs.export.pdf', array_filter(['search' => $search])) }}" class="inline-flex items-center justify-center h-10 px-4 text-sm font-medium text-red-700 bg-red-50 border border-red-200 rounded-lg">PDF</a>@endcanany
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($workLogs as $workLog)
                            <tr>
                                ...
                                <td class="px-5 py-3 text-right text-red-600">{{ number_format((float) $workLog->total_unit_kg - (float) $workLog->received_qty, 2) }}</td>
                                    ...
                                    <div class="flex justify-end gap-2">
                                        @canany(['manage carry man work logs', 'edit carry man work logs'])
                                            <button type="button"
                                                ...
                                                data-action="{{ route('carry-man-work-logs.receive', $workLog) }}"
                                                data-title="{{ $workLog->carryMan?->name ?? '-' }} - {{ $workLog->memo_no ?? 'No memo' }}"
                                                data-total="{{ (float) $workLog->total_unit_kg }}"
                        ...
                        @empty
                            <tr><td colspan="12" class="px-5 py-12 text-center text-gray-400">No work logs found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
