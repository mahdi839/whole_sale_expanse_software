<x-app-layout>
    ...
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            @canany(['manage computer man work logs', 'view computer man work logs'])<a href="{{ route('computer-man-work-logs.export.pdf', array_filter(['search' => $search])) }}" class="inline-flex items-center justify-center h-10 px-4 text-sm font-medium text-red-700 bg-red-50 border border-red-200 rounded-lg">PDF</a>@endcanany
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($workLogs as $workLog)
                            <tr>
                                ...
                                <td class="px-5 py-3 text-right text-red-600">{{ number_format((float) $workLog->computer_design_qty - (float) $workLog->received_qty, 2) }}</td>
                                    ...
                                    <div class="flex justify-end gap-2">
                                        @canany(['manage computer man work logs', 'edit computer man work logs'])
                                            <button type="button"
                                                ...
                                                data-action="{{ route('computer-man-work-logs.receive', $workLog) }}"
                                                data-title="{{ $workLog->computerMan?->name ?? '-' }} - {{ $workLog->product?->product_name ?? '-' }}"
                                                data-total="{{ (float) $workLog->computer_design_qty }}"
                        ...
                        @empty
                            <tr><td colspan="11" class="px-5 py-12 text-center text-gray-400">No work logs found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
