<x-app-layout>
    <x-slot name="header">Received Cloth</x-slot>

    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <form method="GET" action="{{ route('received-cloths.index') }}" class="flex items-center gap-2 w-full sm:max-w-md">
            ...
            @canany(['manage received cloths', 'create received cloths'])
                <a href="{{ route('received-cloths.create') }}" class="inline-flex items-center justify-center h-10 px-4 text-sm font-medium text-white bg-blue-600 rounded-lg">
                    Add Received Cloth
                </a>
            @endcanany
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($receivedCloths as $item)
                            <tr>
                                ...
                                <td class="px-5 py-3">
                                    <div class="flex justify-end gap-2">
                                        @canany(['manage received cloths', 'edit received cloths'])
                                            <a href="{{ route('received-cloths.edit', $item) }}" class="px-3 py-1.5 text-xs text-blue-700 bg-blue-50 rounded-lg">Edit</a>
                                        @endcanany
                                        @canany(['manage received cloths', 'delete received cloths'])
                                            <form method="POST" action="{{ route('received-cloths.destroy', $item) }}" onsubmit="return confirm('Delete this record?')">
                                                @csrf
                                                @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="6" class="px-5 py-12 text-center text-gray-400">No received cloth records found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>

            @if($receivedCloths->hasPages())
                <div class="px-5 py-3 border-t border-gray-100 bg-gray-50/40">{{ $receivedCloths->links() }}</div>
            @endif
        </div>
    </div>
</x-app-layout>
