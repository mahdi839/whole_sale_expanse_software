        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <form method="GET" action="{{ route('customers.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:max-w-md">
                @if(auth()->user()->canManageAllShops())
                    <select name="shop_id" class="px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg"><option value="">All shops</option>@foreach($shops as $shop)<option value="{{ $shop->id }}" @selected($shopId == $shop->id)>{{ $shop->displayLabel() }}</option>@endforeach</select>
                @endif
            ...
            <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <a href="{{ route('customers.export.pdf', request()->only('search')) }}"
                ...
                <a
                    ...
                    class="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition shrink-0"
                >
