...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div
        ...
        @resize.window="drawerOpen = false"
    ...
    >
        ...
        >
            {{-- Logo --}}
            <div class="flex items-center gap-3 px-3.5 py-4 border-b border-white/10 shrink-0 overflow-hidden min-h-[60px]">
                <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-500 shrink-0">
                    <svg class="w-4 h-4 fill-white" viewBox="0 0 24 24">
                        <path d="M20 7H4a2 2 0 00-2 2v10a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2zM4 5h16a1 1 0 000-2H4a1 1 0 000 2z"/>
                    </svg>
                </div>
            ...
            <nav class="flex-1 overflow-y-auto overflow-x-hidden py-3 space-y-0.5 px-2">
                ...
                <p :class="drawerOpen ? 'block' : 'hidden xl:block'"
                   class="px-3 pt-3 pb-1 text-[10px] font-medium uppercase tracking-widest text-white/30">Purchasing</p>
                ...
                <p :class="drawerOpen ? 'block' : 'hidden xl:block'"
                   class="px-3 pt-3 pb-1 text-[10px] font-medium uppercase tracking-widest text-white/30">Reports & Settings</p>
            ...
            <div class="shrink-0 border-t border-white/10 px-2 py-3">
                <div class="flex items-center gap-2.5">
                    ...
                    <form method="POST" action="{{ route('logout') }}"
                        ...
                        @csrf
                        <button type="submit" title="Log Out" class="text-white/30 hover:text-white/70 transition-colors p-1">
        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            ...
                            <div class="flex items-center justify-center w-7 h-7 rounded-full bg-blue-500 text-white text-xs font-medium shrink-0">
                                {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                            </div>
                            <span class="hidden sm:block text-sm text-gray-700 max-w-[120px] truncate">{{ Auth::user()->name }}</span>
                            <svg class="w-3.5 h-3.5 text-gray-400 hidden sm:block" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
                        </button>
            ...
            <nav class="md:hidden fixed bottom-0 inset-x-0 z-30 flex items-center justify-around h-14 bg-[#1e2433] border-t border-white/10">
                ...
                <a href="#"
                   class="flex flex-col items-center gap-0.5 px-3 py-1 {{ request()->routeIs('reports.*') ? 'text-blue-400' : 'text-white/45' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24"><path d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
