...
<head>
    ...
    <style>
        ...
        .sidebar-dropdown-trigger[data-open="true"] {
            background: var(--sidebar-hover-bg);
            color: var(--sidebar-text-bright);
        }
...
<body class="font-sans antialiased">
    ...
    <div x-data="{ drawerOpen: false }" @resize.window="drawerOpen = false" class="flex h-screen overflow-hidden">
        ...
        {{-- ===== SIDEBAR ===== --}}
        <aside id="app-sidebar" :class="drawerOpen ? 'w-64' : 'max-md:-translate-x-full md:w-14 xl:w-64'"
            ...
            <nav class="sidebar-nav">
                ...
                @canany([
                    ...
                    <x-sidebar-dropdown label="Stock Management" :active="request()->routeIs('shops.*') || request()->routeIs('stocks.*')">
                        ...
                        @canany(['manage shops', 'view shops'])
                            <x-sidebar-sub-link :href="route('shops.index')">Shops</x-sidebar-sub-link>
                        @endcanany
                        @canany(['manage stock', 'view stock'])
                            <x-sidebar-sub-link :href="route('stocks.index')">Inventory</x-sidebar-sub-link>
                        @endcanany
                        @can('distribute stock')
                            <x-sidebar-sub-link :href="route('stocks.distribute')">Distribute Stock</x-sidebar-sub-link>
                        @endcan
                        @canany(['view stock distributions', 'receive stock distributions'])
                            <x-sidebar-sub-link :href="route('stocks.distributions.pending')">Pending Stock Receive</x-sidebar-sub-link>
                        @endcanany
                    </x-sidebar-dropdown>
                @endcanany
                ...
                @canany([
                    ...
                    <x-sidebar-dropdown label="Expanse, Cash & cheque" :active="request()->routeIs('expenses.*') ||
                        ...
                        @canany(['manage expenses', 'view expenses'])
                ...
                @canany(['manage employees', 'view employees', 'manage salaries', 'view salaries', 'create salaries',
                    ...
                    <x-sidebar-dropdown label="Salary Management" :active="request()->routeIs('employees.*') ||
                        ...
                        @canany(['manage employees', 'view employees'])
                            <x-sidebar-sub-link :href="route('employees.index')">Employees</x-sidebar-sub-link>
                        @endcanany
                        @canany(['manage salaries', 'view salaries', 'create salaries'])
                            <x-sidebar-sub-link :href="route('salaries.index')">Salaries</x-sidebar-sub-link>
                        @endcanany
                        @canany(['manage salary advances', 'view salary advances', 'create salary advances'])
                            <x-sidebar-sub-link :href="route('salary-advances.index')">Advance Salary</x-sidebar-sub-link>
                        @endcanany
                    </x-sidebar-dropdown>
                @endcanany
                ...
                @if(Auth::user()->canManageAllShops())
                    <div class="mb-2">
                        <a href="{{ route('admin.database-backup') }}"
                            ...
                            <span :class="drawerOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0 xl:opacity-100 xl:w-auto'"
        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            ...
                            <div
                                class="flex items-center justify-center w-7 h-7 rounded-full bg-indigo-600 text-white text-xs font-medium shrink-0">
                                {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                            </div>
                            <span
            ...
            <nav
                class="md:hidden fixed bottom-0 inset-x-0 z-30 flex items-center justify-around h-14 bg-[#1a1360] border-t border-white/10">
                <a href="{{ route('dashboard') }}"
                    class="flex flex-col items-center gap-0.5 px-3 py-1 {{ request()->routeIs('dashboard') ? 'text-violet-400' : 'text-white/40' }}">
                ...
                <a href="{{ route('purchases.index') }}"
                    class="flex flex-col items-center gap-0.5 px-3 py-1 {{ request()->routeIs('purchases.*') ? 'text-violet-400' : 'text-white/40' }}">
                ...
                <a href="{{ route('sales.index') }}"
                    class="flex flex-col items-center gap-0.5 px-3 py-1 {{ request()->routeIs('sales.*') ? 'text-violet-400' : 'text-white/40' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.75"
                        viewBox="0 0 24 24">
                        <path
                            d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                ...
                <button @click="drawerOpen = !drawerOpen"
                    class="flex flex-col items-center gap-0.5 px-3 py-1 text-white/40">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.75"
                        viewBox="0 0 24 24">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll(
                'select[name*="customer_id"], select[name*="supplier_id"], select[name*="product_id"], select.tom-select'
            ).forEach((select) => {
