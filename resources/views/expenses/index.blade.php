            ...
            <form method="GET" action="{{ route('expenses.index') }}">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 mb-3.5">
                    @if (auth()->user()->canManageAllShops())
                        <div><label class="block text-xs font-medium text-gray-500 mb-1">Shop</label><select
                                name="shop_id"
                                class="h-10 px-3 text-sm bg-gray-50 border border-gray-200 rounded-lg w-full">
                                <option value="">All shops</option>
                                @foreach ($shops as $shop)
                                    <option value="{{ $shop->id }}" @selected(($filters['shop_id'] ?? null) == $shop->id)>{{ $shop->displayLabel() }}
                                    </option>
                                @endforeach
                            </select></div>
                    @endif
                ...
                <div class="flex flex-col sm:flex-row sm:flex-wrap gap-2">
                    <button type="submit" class="h-10 px-4 bg-gray-800 text-white rounded-lg text-sm">
                        Filter
                    </button>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($expenses as $expense)
                            <tr class="hover:bg-gray-50/60">
                                ...
                                <td class="px-5 py-3 ">
                                  <span class="p-2 rounded {{ $expense?->shop?->id === 1 ? 'bg-blue-300' : 'bg-green-300' }}">{{ $expense?->shop?->name??"No Shop Found" }}</span>  
                                </td>
                                <td class="px-5 py-3">
                                    {{ $expense->category }}
                                </td>
